...
def create_seat(seat: List[SeatCreate], db: Session = Depends(get_db)):
    """Create a new seat"""
...
def get_all_seats(db: Session = Depends(get_db), skip: int = 0, limit: int = 10):
    """Fetch all seats"""
...
def update_seat(seat_id: int, seat: SeatUpdate, db: Session = Depends(get_db)):
    """Update seat details"""
    db_seat = seat_crud.get(db=db, id=seat_id)
    if not db_seat:
        raise HTTPException(status_code=404, detail="Seat not found")
