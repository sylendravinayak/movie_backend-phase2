...
def calculate_demand_surge(current_occ: float) -> float:
    """
    ...
    if effective_occ >= HIGH_DEMAND_THRESHOLD:
        ...
        discount = 1.0 - (LOW_DEMAND_THRESHOLD - effective_occ) * 0.6
        return max(discount, 0.85)
...
def pricing_node(state: OpsState):
    ...
    for show_id in show_ids:
        ...
        forecast_demand = forecast.get("forecast_demand")
        forecast_occ = forecast_demand / capacity
        effective_occ = min(
    (forecast_occ * 0.7) + (current_occ * 0.3),
    1.0
) 
        show_datetime = datetime.combine(show.show_date, show.show_time)
        demand_surge = calculate_demand_surge(effective_occ)
        ...
        base_multiplier = demand_surge * slot_mult * day_mult
        base_multiplier = min(base_multiplier, 1.25)
        price_multiplier = base_multiplier * time_urgency
